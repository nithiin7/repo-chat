    ...
    return (
      <div className="flex flex-col items-center gap-4 py-20 text-center">
        <motion.div
          ...
          className="border-border bg-card flex size-16 items-center justify-center rounded-2xl border shadow-sm"
        >
        ...
        >
          <p className="text-foreground font-semibold">No repositories indexed yet</p>
    ...
    <section>
      ...
      >
        <div className="flex items-center gap-2.5">
          <h2 className="text-foreground font-semibold">Your Repositories</h2>
      ...
      {repos.length >= 2 && selectedIds.size === 0 && (
        <motion.p
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          className="text-muted-foreground/60 mb-4 flex items-center gap-1.5 text-xs"
        >
      ...
      {compareOpen && (
        <MultiRepoChatDialog repos={selectedRepos} onClose={() => setCompareOpen(false)} />
      )}
    </section>
  );
};
