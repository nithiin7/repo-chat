...
const ChatEmptyState = ({ repo, mode, onSuggestionClick }: ChatEmptyStateProps) => (
  <div className="flex h-full flex-col items-center justify-center gap-6 px-4 pt-8 pb-16 text-center">
    <motion.div
      initial={{ opacity: 0, scale: 0.8 }}
      animate={{ opacity: 1, scale: 1 }}
      transition={{ duration: 0.4, ease: [0.25, 0.1, 0.25, 1] }}
      className="border-border bg-card flex size-14 items-center justify-center rounded-2xl border text-indigo-400"
    >
    ...
    >
      <p className="text-foreground font-semibold">Ask anything about this codebase</p>
    ...
    >
      ...
      <div>
        <p className="text-foreground text-sm font-medium">Generate onboarding doc</p>
        <p className="text-muted-foreground text-xs">One-click summary for new developers</p>
      </div>
    </motion.button>
    ...
    <div className="grid w-full max-w-lg grid-cols-1 gap-2 sm:grid-cols-2">
      {SUGGESTIONS.map((s, i) => (
        <motion.button
          ...
          className="border-border bg-card/60 text-muted-foreground hover:bg-card hover:text-foreground cursor-pointer rounded-xl border px-4 py-3 text-left text-sm transition-[border-color,background-color,color] duration-150 hover:border-indigo-500/30"
        >
          {s}
        </motion.button>
      ))}
