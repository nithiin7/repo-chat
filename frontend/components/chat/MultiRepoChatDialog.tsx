  ...
  return (
    <AnimatePresence>
      ...
      <motion.div
        ...
        className="border-border bg-background fixed inset-4 z-50 flex flex-col overflow-hidden rounded-2xl border shadow-2xl sm:inset-8 md:inset-12 lg:inset-16"
        ...
        <div className="border-border flex shrink-0 items-center justify-between gap-4 border-b px-5 py-3.5">
          <div className="flex min-w-0 items-center gap-3">
            ...
            <div className="min-w-0">
              <p className="text-foreground text-sm font-semibold">Cross-repo comparison</p>
              <div className="mt-0.5 flex flex-wrap gap-1.5">
                {repos.map((r) => {
                  ...
                  return (
                    <span
                      key={r.repo_id}
                      className="rounded-full bg-indigo-500/10 px-2 py-0.5 font-mono text-[10px] font-medium text-indigo-400"
                    >
                      {name}
                    </span>
                  );
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              disabled={streaming}
              className="text-muted-foreground hover:bg-muted hover:text-foreground flex size-8 cursor-pointer items-center justify-center rounded-lg transition-colors disabled:pointer-events-none disabled:opacity-40"
            >
              <X className="size-4" />
            </button>
          </div>
        ...
        <div ref={scrollRef} onScroll={handleScroll} className="flex-1 overflow-y-auto">
          {messages.length === 0 ? (
            <div className="flex h-full flex-col items-center justify-center gap-3 px-6 py-16 text-center">
              <GitCompare className="text-muted-foreground/30 size-10" />
              <p className="text-foreground font-semibold">Ask a cross-cutting question</p>
              <p className="text-muted-foreground max-w-sm text-sm">
                Compare implementations, patterns, or design decisions across the selected
                repositories.
              </p>
              <div className="mt-2 flex flex-wrap justify-center gap-2">
                {[
                  ...
                  <button
                    key={s}
                    onClick={() => submit(s)}
                    className="border-border bg-card text-muted-foreground hover:text-foreground cursor-pointer rounded-lg border px-3 py-1.5 text-xs transition-colors hover:border-indigo-500/30"
                  >
                    {s}
                  </button>
                ))}
