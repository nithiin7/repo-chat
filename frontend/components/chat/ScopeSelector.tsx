...
export default function ScopeSelector({ repoId, scopePath, onChange, disabled }: ScopeSelectorProps) {
  ...
  useEffect(() => {
    ...
    getRepoFiles(repoId).then(setFiles).catch(() => setFiles([]))
  }, [open, files, repoId])
  ...
  return (
    <div ref={containerRef} className="relative">
      ...
      >
        {scopePath ? <FolderOpen className="size-3 shrink-0" /> : <Folder className="size-3 shrink-0" />}
      ...
      {open && (
        <div className="absolute bottom-full left-0 z-30 mb-2 w-80 overflow-hidden rounded-lg border border-border bg-popover shadow-lg">
          ...
          {scopePath && (
            <div className="border-t border-border px-3 py-2">
              <button
                ...
                className="w-full cursor-pointer rounded-md py-1 text-xs text-muted-foreground transition-colors hover:text-foreground"
              >
                Clear scope
              </button>
            </div>
