...
const LandingPage = () => {
  return (
    <div className="relative flex min-h-screen flex-col">
      ...
      {/* hero */}
      <header className="relative px-4 pt-20 pb-24 text-center">
        <motion.div
          initial={{ opacity: 0, y: 14 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.5, ease }}
          className="border-border bg-muted text-muted-foreground mx-auto mb-8 inline-flex items-center gap-2 rounded-full border px-3.5 py-1.5 text-xs font-medium tracking-wide"
        >
        ...
        <motion.h1
          ...
          className="from-foreground to-foreground/40 bg-linear-to-b bg-clip-text text-5xl font-bold tracking-tight text-transparent sm:text-6xl dark:from-white dark:via-white/90 dark:to-white/40"
        >
          CodeLens
        </motion.h1>
      ...
      <main className="mx-auto w-full max-w-5xl flex-1 px-4 pb-24 sm:px-6 lg:px-10">
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {FEATURES.map((f, i) => (
            ...
            >
              <div className="border-border bg-muted mb-3 flex size-9 items-center justify-center rounded-lg border">
                {f.icon}
              </div>
      ...
      {/* footer */}
      <footer className="border-border/40 text-muted-foreground/60 border-t py-6 text-center text-xs">
